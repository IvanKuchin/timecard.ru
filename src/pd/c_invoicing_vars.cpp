...
auto	C_Invoicing_Vars::FillStaticDictionary() -> string
{
	...
	if(error_message.empty()) error_message = AssignVariableValue("vat_correction_1", 		gettext("vat_correction_1"), true);
	if(error_message.empty()) error_message = AssignVariableValue("vat_correction_2", 		gettext("vat_correction_2"), true);
	...
	if(error_message.empty()) error_message = AssignVariableValue("vat_shipping_doc",		gettext("vat_shipping_doc"), true);
	if(error_message.empty()) error_message = AssignVariableValue("VAT title - Payment order number",   gettext("VAT title - Payment order number"), true);
	...
	if(error_message.empty()) error_message = AssignVariableValue("VAT comment4",						gettext("VAT comment4"), true);
		...
		if(db)
		{
			...
			if(error_message.empty())
			{
				...
				if(Get("1C_template_invoice_to_cc_service").length())
					AssignVariableValue("1C_template_invoice_to_cc_service_full_path", config->GetFromFile("image_folders", "template_costcenter") + Get("1C_template_invoice_to_cc_service"), true);
				if(Get("1C_template_invoice_to_cc_service_table_row").length())
					AssignVariableValue("1C_template_invoice_to_cc_service_table_row_full_path", config->GetFromFile("image_folders", "template_costcenter") + Get("1C_template_invoice_to_cc_service_table_row"), true);
				
				if(Get("1C_template_invoice_to_cc_bt").length())
					AssignVariableValue("1C_template_invoice_to_cc_bt_full_path", config->GetFromFile("image_folders", "template_costcenter") + Get("1C_template_invoice_to_cc_bt"), true);
				if(Get("1C_template_invoice_to_cc_bt_table_row").length())
					AssignVariableValue("1C_template_invoice_to_cc_bt_table_row_full_path", config->GetFromFile("image_folders", "template_costcenter") + Get("1C_template_invoice_to_cc_bt_table_row"), true);
			}
		}
		else
		{
		...
		if(db)
		{
			...
			if(error_message.empty())
			{
				...
				if(Get("1C_template_payment_to_subcontractor").length())
					AssignVariableValue("1C_template_payment_to_subcontractor_full_path", config->GetFromFile("image_folders", "template_company") + Get("1C_template_payment_to_subcontractor"), true);
				if(Get("1C_template_payment_to_subcontractor_body").length())
					AssignVariableValue("1C_template_payment_to_subcontractor_body_full_path", config->GetFromFile("image_folders", "template_company") + Get("1C_template_payment_to_subcontractor_body"), true);
				
				if(Get("1C_template_payment_order_to_subcontractor").length())
					AssignVariableValue("1C_template_payment_order_to_subcontractor_full_path", config->GetFromFile("image_folders", "template_company") + Get("1C_template_payment_order_to_subcontractor"), true);
				if(Get("1C_template_payment_order_to_subcontractor_body").length())
					AssignVariableValue("1C_template_payment_order_to_subcontractor_body_full_path", config->GetFromFile("image_folders", "template_company") + Get("1C_template_payment_order_to_subcontractor_body"), true);
