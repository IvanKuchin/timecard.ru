...
auto C_Invoice_Service_Subc_To_Agency::GenerateDocumentArchive() -> string
{
	...
	if(error_message.empty())
	{
		...
		for(auto &timecard: timecard_obj_list)
		{
			...
			timecard_printer.xlSetKey(config->GetFromFile(SECRET_FILE, "XL_USERNAME"), config->GetFromFile(SECRET_FILE, "XL_KEY"));
											...
											");"))
								{
												...
												");"))
									{
										...
										// --- override agency roles with SoW-specific roles
										{
											...
											temp1 = GetValueFromDB("SELECT `value` FROM `contract_sow_custom_fields` WHERE `contract_sow_id`=" + quoted(sow_id) + " AND `var_name`=\"subc2agency_timecard_position1\";", db);
											temp2 = GetValueFromDB("SELECT `value` FROM `contract_sow_custom_fields` WHERE `contract_sow_id`=" + quoted(sow_id) + " AND `var_name`=\"subc2agency_timecard_signature1\";", db);
											...
											temp1 = GetValueFromDB("SELECT `value` FROM `contract_sow_custom_fields` WHERE `contract_sow_id`=" + quoted(sow_id) + " AND `var_name`=\"subc2agency_timecard_position2\";", db);
											temp2 = GetValueFromDB("SELECT `value` FROM `contract_sow_custom_fields` WHERE `contract_sow_id`=" + quoted(sow_id) + " AND `var_name`=\"subc2agency_timecard_signature2\";", db);
											if(temp1.length() + temp2.length())
											{
